...
function renderVerseContent(content) {
  return content.map((item, i) => {
    ...
    if (item.text) {
      ...
      if (item.poem)         return <span key={i} className="block pl-4 italic text-primary">{item.text}</span>;
      return <span key={i}>{item.text} </span>;
    }
    if (item.heading)   return <span key={i} className="block font-cormorant text-lg font-semibold text-primary mt-4 mb-1">{item.heading}</span>;
    ...
    <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2.5 px-4 py-2.5 border shadow-lg text-xs font-coptic tracking-widest uppercase transition-all
      ${type === 'error'  ? 'bg-red-50 border-red-200 text-red-600' :
        type === 'exists' ? 'bg-amber-50 border-amber-200 text-amber-700' :
                            'bg-ivory border-divider text-primary'}`}
    >
  ...
  return (
    <p
      ...
      className={`group relative leading-loose text-base font-serif cursor-pointer rounded px-2 py-0.5 transition-colors ${
        isHighlighted ? 'bg-amber-100 border-l-2 border-amber-500 pr-10' : 'hover:bg-ivory'
      }`}
    >
        ...
        <button
          ...
          className={`absolute right-2 top-1/2 -translate-y-1/2 p-1 border transition-colors
            ${isSaved
              ...
              : 'text-primary border-divider hover:text-amber-500 hover:border-amber-300 hover:bg-amber-50'
            }`}
        >
          {isBookmarking
...
function BookList({ books, selectedBook, loadingBooks, bookSearch, setBookSearch, onSelectBook }) {
  ...
  return (
    <>
      <div className="px-3 py-2.5 border-b border-divider shrink-0">
        <div className="relative">
          <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-primary"><IconSearch /></span>
          <input
            type="text"
            value={bookSearch}
            onChange={e => setBookSearch(e.target.value)}
            placeholder="Find book…"
            className="w-full bg-ivory border border-divider focus:border-amber-400 focus:outline-none pl-8 pr-3 py-1.5 text-[0.9rem] text-primary placeholder:text-secondary"
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-2">
        {loadingBooks ? (
          ...
          <>
            {filteredOT.length > 0 && (
              <>
                <p className="font-coptic text-[0.65rem] uppercase tracking-[0.2em] text-secondary px-4 pt-3 pb-1">Old Testament</p>
                {filteredOT.map(book => (
                  <button key={book.id} onClick={() => onSelectBook(book)}
                    className={`w-full text-left px-4 py-2 font-cormorant text-md font-semibold transition-colors border-l-2 ${
                      selectedBook?.id === book.id
                        ...
                        : 'text-primary hover:text-amber-600 hover:bg-amber-50 border-l-transparent'
                    }`}
                  >
                    {book.name}
                    <span className="ml-1 font-coptic text-[0.65rem] text-primary">{book.numberOfChapters}ch</span>
                  </button>
                ))}
              </>
            )}
            {filteredNT.length > 0 && (
              <>
                <p className="font-coptic text-[0.65rem] uppercase tracking-[0.2em] text-secondary px-4 pt-4 pb-1">New Testament</p>
                {filteredNT.map(book => (
                  <button key={book.id} onClick={() => onSelectBook(book)}
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-ivory">
      ...
      <div className="flex flex-1 min-w-0 overflow-hidden relative">
        ...
        {/* DESKTOP book panel */}
        <aside className={`hidden lg:flex flex-col bg-porcelain border-r border-divider h-screen overflow-hidden transition-all duration-300 ${showBookPanel ? 'w-56 shrink-0' : 'w-0'}`}>
          <div className="px-4 py-5 border-b border-divider shrink-0">
            <p className="font-coptic text-[0.5rem] uppercase tracking-[0.25em] text-secondary mb-1">Scripture</p>
            <h2 className="font-cormorant text-xl font-semibold text-strong">Books</h2>
        ...
        <div className={`lg:hidden fixed z-50 bg-porcelain flex flex-col transition-transform duration-300
          bottom-0 left-0 right-0 h-[75vh] rounded-t-2xl border-t border-divider
          md:bottom-auto md:top-0 md:right-auto md:h-full md:w-72 md:rounded-none md:border-t-0 md:border-r md:border-divider
        ...
        >
          ...
          <div className="px-4 py-4 border-b border-divider shrink-0 flex items-center justify-between">
            <div>
              <p className="font-coptic text-[0.5rem] uppercase tracking-[0.25em] text-secondary mb-0.5">Scripture</p>
              <h2 className="font-cormorant text-xl font-semibold text-strong">Books</h2>
              <div className="w-5 h-0.5 bg-amber-500 mt-1.5" />
            </div>
            <button onClick={() => setShowBookPanel(false)}
              className="p-1.5 text-secondary hover:text-strong border border-divider hover:border-gray-300 transition-colors"
            >
              <IconX />
            </button>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          {/* Top bar */}
          <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-5 py-3 sm:py-3.5 border-b border-divider bg-porcelain shrink-0">
            <button onClick={() => setShowBookPanel(v => !v)}
              className="p-1.5 text-primary hover:text-amber-500 border border-divider hover:border-amber-300 transition-colors shrink-0"
            >
            ...
            <div className="flex items-center gap-1.5 flex-1 min-w-0">
              {selectedBook ? (
                <>
                  <span className="font-cormorant text-base sm:text-lg font-semibold text-strong truncate">{selectedBook.name}</span>
                  <span className="text-placeholder-primary shrink-0"><IconChevron dir="right" /></span>
                  <span className="font-coptic text-[0.6rem] uppercase tracking-widest text-amber-600 shrink-0">Ch. {chapterNum}</span>
                </>
              ) : (
                <span className="font-cormorant text-base sm:text-lg text-primary truncate">Select a book to begin</span>
              )}
            </div>
            ...
            <button
              ...
              className="relative flex items-center gap-1.5 border border-divider hover:border-amber-300 px-2.5 py-1.5 text-primary hover:text-amber-600 transition-colors shrink-0"
            >
              <IconBrain />
            ...
            <div className="relative shrink-0">
              <button onClick={() => setShowTransDD(v => !v)}
                className="flex items-center gap-1.5 sm:gap-2 border border-divider hover:border-amber-300 px-2 sm:px-3 py-1.5 transition-colors"
              >
                <span className="font-coptic text-[0.6rem] uppercase tracking-widest text-amber-600">{translation}</span>
                <span className="text-primary"><IconChevron dir="down" /></span>
              </button>
              {showTransDD && (
                <div className="absolute right-0 top-full mt-1 w-64 sm:w-72 bg-ivory border border-divider shadow-lg z-30 flex flex-col max-h-72 sm:max-h-80">
                  <div className="p-2 border-b border-divider">
                    <input autoFocus type="text" value={transSearch} onChange={e => setTransSearch(e.target.value)}
                      placeholder="Search translations…"
                      className="w-full text-xs border border-divider focus:border-amber-400 focus:outline-none px-3 py-1.5 text-primary placeholder:text-placeholder-primary"
                    />
                  </div>
                  <div className="overflow-y-auto flex-1">
                    {filteredTranslations.slice(0, 40).map(t => (
                      <button key={t.id}
                        ...
                        className={`w-full text-left px-4 py-2.5 hover:bg-amber-50 transition-colors border-b border-divider ${t.id === translation ? 'bg-amber-50' : ''}`}
                      >
                        <span className="font-coptic text-[0.7rem] uppercase tracking-widest text-amber-600 block">{t.shortName}</span>
                        <span className="font-cormorant text-sm text-primary">{t.englishName}</span>
                      </button>
                    ))}
                  </div>
                </div>
          ...
          {selectedBook && !loadingChapter && chapterData && (
            <div className="border-b border-divider bg-ivory px-3 sm:px-5 py-2 shrink-0 overflow-x-auto">
              <div className="flex items-center gap-1">
                {chapterNums.map(n => (
                  <button key={n} onClick={() => setChapterNum(n)}
                    className={`w-7 h-7 text-md font-cormorant transition-colors shrink-0 ${
                      n === chapterNum ? 'bg-amber-500 text-white' : 'text-strong hover:bg-amber-100 hover:text-amber-700'
                    }`}
                  >{n}</button>
                ))}
              </div>
          ...
          <div ref={contentRef} className="flex-1 overflow-y-auto px-4 sm:px-8 lg:px-10 py-6 sm:py-8 bg-ivory">
            ...
            {!selectedBook && !loadingBooks && (
              <div className="flex flex-col items-center justify-center h-full gap-4 text-center px-4">
                <div className="w-14 h-14 border border-divider flex items-center justify-center text-placeholder-primary"><IconBook /></div>
                <p className="font-cormorant text-2xl text-primary">Open a book to begin</p>
                <p className="font-coptic text-[0.7rem] uppercase tracking-widest text-primary">Choose from the books list</p>
            ...
            {loadingChapter && (
              <div className="flex items-center justify-center h-full gap-3">
                <div className="w-5 h-5 border border-amber-500/40 border-t-amber-500 rounded-full animate-spin" />
                <p className="font-coptic text-[0.7rem] uppercase tracking-widest text-primary">Loading…</p>
              </div>
            )}
            ...
            {chapterData && !loadingChapter && (
              <div className="max-w-2xl mx-auto">
                <div className="mb-6 sm:mb-8">
                  ...
                  <h1 className="font-cormorant text-3xl sm:text-4xl font-light text-strong">
                ...
                <div className="space-y-1 text-primary">
                  {chapterData.chapter.content.map((item, i) => {
                    if (item.type === 'heading') return (
                      <h3 key={i} className="font-cormorant text-xl font-semibold text-primary pt-6 pb-1">
                        {item.content.join(' ')}
                      </h3>
                    );
                    ...
                    if (item.type === 'hebrew_subtitle') return (
                      <p key={i} className="font-cormorant italic text-primary text-sm mb-3 border-l-2 border-amber-300 pl-3">
                ...
                {footnotes.length > 0 && (
                  <div className="mt-10 border-t border-divider pt-5">
                    <button onClick={() => setShowFootnotes(v => !v)}
                      className="flex items-center gap-2 font-coptic text-[0.7rem] uppercase tracking-widest text-primary hover:text-amber-600 transition-colors mb-3"
                    >
                    ...
                    {showFootnotes && (
                      <div className="space-y-2">
                        {footnotes.map((fn, i) => (
                          <p key={i} className="text-xs text-primary font-coptic leading-relaxed">
                ...
                <div className="flex items-center justify-between mt-10 sm:mt-12 pt-6 border-t border-divider">
                  <button onClick={() => goChapter(-1)} disabled={!prevChapter}
                    className="flex items-center gap-2 font-coptic text-[0.6rem] uppercase tracking-widest text-primary hover:text-amber-600 disabled:opacity-20 disabled:cursor-not-allowed transition-colors border border-divider hover:border-amber-300 disabled:hover:border-divider px-3 sm:px-4 py-2"
                  >
                    <IconChevron dir="left" />
                    <span className="hidden sm:inline">Previous</span>
                  </button>
                  <span className="font-coptic text-[0.7rem] uppercase tracking-widest text-strong font-semibold">
