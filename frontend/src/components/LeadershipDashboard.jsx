  ...
  return (
    ...
    >
      <div className="flex items-start justify-between p-5 border-b border-divider">
        ...
        <span className="text-sm uppercase tracking-widest text-amber-700 bg-amber-50 border border-amber-100 px-2 py-1 shrink-0 ml-3">
...
function CreateTeamForm({ allUsers, allProfiles, onSuccess }) {
  const createLeadershipTeam = useMainStore(
    (state) => state.createLeadershipTeam,
  );
...
function EditTeamForm({ team, allUsers, allProfiles, onSuccess, onCancel }) {
  const updateLeadershipTeam = useMainStore(
    (state) => state.updateLeadershipTeam,
  );
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name.trim()) return setError("Team name is required.");
    setSubmitting(true);
    const result = await updateLeadershipTeam(team.id, {
      name,
      description,
      members: selectedMembers,
    });
...
function LeadershipDashboard() {
  ...
  const fetchLeadershipTeams = useMainStore(
    (state) => state.fetchLeadershipTeams,
  );
  ...
  const totalMembers = teamList.reduce(
            ...
            <>
              ...
              {mode === "create" && (
                <CreateTeamForm
                  ...
                  onSuccess={() => {
                    setMode("idle");
                    showSuccess("Leadership team created successfully ✓");
                  }}
                />
              )}
              {mode === "edit" && selected && (
