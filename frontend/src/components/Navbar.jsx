...
function Navbar() {
  ...
  return (
    <div className="fixed w-full top-0 z-50">
      <nav className="w-full flex items-center justify-between bg-porcelain/95 backdrop-blur-md border-b border-stone-200 px-6 sm:px-10 py-4">
        {/* Logo */}
        <NavLink
          to="/"
        ...
        <ul className="hidden sm:flex items-center gap-1">
          <li>
            <a
              href="#"
              onClick={handleHome}
              className="font-coptic text-[0.6rem] uppercase tracking-[0.2em] transition-colors px-3 py-1.5 border-b border-transparent text-stone-700 hover:text-black"
            >
              Home
            </a>
          </li>
        ...
        >
          <span
            className={`block w-5 h-px bg-stone-700 transition-all duration-200 ${mobileOpen ? "rotate-45 translate-y-[3px]" : ""}`}
          />
          <span
            className={`block w-5 h-px bg-stone-700 transition-all duration-200 ${mobileOpen ? "opacity-0" : ""}`}
          />
          <span
            className={`block w-5 h-px bg-stone-700 transition-all duration-200 ${mobileOpen ? "-rotate-45 -translate-y-[3px]" : ""}`}
          />
      ...
      {/* Mobile menu */}
      <div
        className={`sm:hidden bg-porcelain border-b border-stone-200 flex flex-col transition-all duration-200 overflow-hidden ${
          mobileOpen
