...
function CharityCard({ org, allUsers, allProfiles, onSelect, isSelected }) {
  ...
  return (
    <div onClick={() => onSelect(org)}
      ...
      {org.banner && (
        <div className="w-full h-28 overflow-hidden border-b border-divider">
      ...
      <div className="flex items-start justify-between p-5 border-b border-divider">
        <div className="flex-1 min-w-0">
          <h3 className="font-cormorant text-xl font-semibold text-strong leading-tight truncate">{org.name}</h3>
          {org.description && <p className="text-xs text-primary mt-1 line-clamp-2">{org.description}</p>}
        </div>
      </div>
      {pastor && (
            ...
            return (
              <>
                ...
                <div>
                  ...
                  <p className="text-xs text-secondary">{userName(pastor)}</p>
                </div>
              </>
            );
          })()}
      ...
      <div className="px-5 py-3 flex items-center justify-between">
        ...
        {org.donation_link && (
          <span className="flex items-center gap-1 text-xs uppercase tracking-widest text-amber-600">
            <IconLink /> Donate
          </span>
        )}
      </div>
...
function CharityDetail({ org, allUsers, allProfiles, onClose, onEdit, onDelete }) {
  ...
  return (
    <div className="relative overflow-hidden bg-amber-50 border border-amber-200 rounded-sm shadow-sm p-6 flex flex-col gap-5">
      ...
      {org.banner && (
        <div className="w-full h-32 overflow-hidden rounded-sm -mx-0 -mt-0 border border-divider">
          <img src={org.banner} alt={org.name} className="w-full h-full object-cover" />
        </div>
      )}
      ...
      <div className="flex items-start justify-between relative z-10">
        <div className="flex-1 min-w-0">
          <p className="text-sm uppercase tracking-[0.25em] text-primary mb-1">Charity Organisation</p>
          <h3 className="font-cormorant text-2xl font-semibold text-strong leading-tight truncate">{org.name}</h3>
        </div>
        <div className="flex items-center gap-0.5 shrink-0 ml-2">
          ...
          {!confirmDelete ? (
            ...
            <div className="flex items-center gap-1 bg-red-50 border border-red-200 px-2 py-1 ml-1">
              ...
              <button onClick={() => onDelete(org.id)} className="p-0.5 text-red-700 hover:text-red-700"><IconCheck /></button>
      ...
      {org.description && (
        <p className="text-sm text-primary leading-relaxed relative z-10 border-l-2 border-amber-500/35 pl-3">{org.description}</p>
      )}

      <div className="relative z-10 flex flex-wrap gap-3">
        {org.payment_method && (
          <div className="w-full">
            <p className="text-xs uppercase tracking-widest text-primary font-coptic mb-1">Payment Details</p>
            <p className="text-xs text-primary whitespace-pre-wrap leading-relaxed">{org.payment_method}</p>
          </div>
        )}
        {org.donation_link && (
          <div>
            <p className="text-xs uppercase tracking-widest text-primary font-coptic mb-1">Donation Link</p>
...
function CharityFields({
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-1.5">
        <InputLabel>Banner Image <span className="text-secondary normal-case tracking-normal">(optional)</span></InputLabel>
        {existingBanner && !bannerFile && (
          <div className="w-full h-20 border border-divider overflow-hidden mb-1">
        ...
        <button type="button" onClick={() => fileInputRef.current?.click()}
          className="flex items-center gap-2 border border-dashed border-divider hover:border-amber-400 text-primary hover:text-amber-500 text-xs px-3 py-2.5 transition-colors w-full justify-center">
          <IconPhoto />
        ...
        {bannerFile && (
          <button type="button" onClick={() => setBannerFile(null)}
            className="text-xs uppercase tracking-widest text-primary hover:text-red-700 transition-colors text-left">
            Remove selection
          </button>
        )}
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        <InputLabel>Payment Details <span className="text-secondary normal-case tracking-normal">(optional)</span></InputLabel>
        <textarea value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)}
          ...
          className="bg-ivory border border-divider focus:border-amber-400 focus:outline-none px-3 py-2.5 text-sm text-primary placeholder:text-primary transition-colors resize-none min-h-20 w-full" />
      </div>

      <div className="flex flex-col gap-1.5">
        <InputLabel>Donation Link <span className="text-secondary normal-case tracking-normal">(optional)</span></InputLabel>
  ...
  return (
    <div className="min-h-screen w-full bg-ivory p-8">
      ...
      <div className="flex items-end justify-between mb-10">
        <div>
          <p className="text-xs uppercase tracking-[0.25em] text-primary mb-1">Outreach</p>
          <h1 className="font-cormorant text-4xl font-semibold text-black leading-tight">Charity Organisations</h1>
